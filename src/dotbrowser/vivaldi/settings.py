"""Manage Vivaldi general settings via a TOML config file.

Vivaldi is Chromium-based, so its `Preferences` JSON has the same
"tracked prefs / `protection.macs` MAC integrity" model as Brave.
This module is the parallel of `brave/settings.py` and the load-bearing
logic is identical: dotted-path keys, refuse anything that has a
sibling under `protection.macs.<dotted_path>`, refuse the whole
`protection.*` subtree.

Config schema (TOML), inside the unified `vivaldi.toml`:

    [settings]
    "vivaldi.tabs.preview_thumbnail_zoom_level" = 1
    "browser.show_home_button" = true   # will be REFUSED on profiles
                                         # where this key is MAC-tracked
...
- `plan_apply(prefs_path, prefs, raw_table)` — pure: builds a `Plan` from
  a parsed `[settings]` table. Used by the unified `vivaldi apply` runner.
- CLI sub-action `dump` (read-only). Apply lives at the `vivaldi` level.
"""
...
def _pop_value(prefs: dict, parts: tuple[str, ...]) -> None:
    ...
    We do not garbage-collect empty parent dicts: Vivaldi tolerates them
    fine, and pruning could race with siblings written by Vivaldi itself
    ...
    if args.keys:
        ...
        if not keys:
            sys.exit(
                ...
                "Pass keys explicitly: `dump vivaldi.tabs.preview_thumbnail_zoom_level ...`"
            )

    lines = ["# Generated by `dotbrowser vivaldi settings dump`", "[settings]"]
    missing: list[str] = []
    for key in keys:
...
def register(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "settings",
        help="inspect general settings (apply lives at `vivaldi apply`)",
    )
